import React from 'react';
import {
  View,
  Text,
  Button,
  AsyncStorage
} from 'react-native';
import axios from 'axios';

const TestScreen = () => {

  const companys = {
    ups: {
      drivers: [
        {
          name: 'bari',
          lastName: 'yuda brother',
          age: '56',
          id: '3125687898',
          licenceNum: '4R55T4S5R4T',
          licenceType: '2',
          phoneNumber: '03001231213',
          mail: 'bari@gmail.com',
          driverID: 'B1321',
          userID: 'XmZla6C110ejpBAxlS3b30mYDYR2',
          tripStatus: false
        },
        {
          name: 'yuda',
          lastName: 'sapar',
          age: '45',
          id: '312897456',
          licenceNum: '4Y44R4S4FG4',
          licenceType: '2',
          phoneNumber: '03001231415',
          mail: 'yuda@gmail.com',
          driverID: 'B1232',
          userID: 'RlPSssz2OGeOWm5A2x3drGRGAqi2',
          tripStatus: false
        },
        {
          name: 'kai',
          lastName: 'malginov',
          age: '58',
          id: '3125687888',
          licenceNum: '2W31G35E4RD',
          licenceType: '2',
          phoneNumber: '03001231617',
          mail: 'vavae@gmail.com',
          driverID: 'B2121',
          userID: '',
          tripStatus: false
        },
        {
          name: 'rafi',
          lastName: 'bazumet',
          age: '63',
          id: '3125687895',
          licenceNum: '32G123S524G',
          licenceType: '1',
          phoneNumber: '03001231819',
          mail: 'maya@gmail.com',
          driverID: 'B4545',
          userID: '',
          tripStatus: false
        },
        {
          name: 'shlomo',
          lastName: 'daniel',
          age: '77',
          id: '012589648',
          licenceNum: 'S3R2G1S23R5',
          licenceType: '1',
          phoneNumber: '03001232122',
          mail: 'namaya@gmail.com',
          driverID: 'B4857',
          userID: '',
          tripStatus: false
        },
        {
          name: 'roni',
          lastName: 'shalev',
          age: '23',
          id: '014785859',
          licenceNum: 'SR6541G26S5',
          licenceType: '2',
          phoneNumber: '03001232324',
          mail: 'nodemand@gmail.com',
          driverID: 'B9878',
          userID: '',
          tripStatus: false
        },
        {
          name: 'yoni',
          lastName: 'cohen',
          age: '34',
          id: '165859652',
          licenceNum: '15SR63G15GG',
          licenceType: '2',
          phoneNumber: '03001232526',
          mail: 'noobfotall@gmail.com',
          driverID: 'B5465',
          userID: '',
          tripStatus: false
        },
        {
          name: 'avraham',
          lastName: 'mrfison',
          age: '42',
          id: '456969631',
          licenceNum: '35ES41G3625',
          licenceType: '3',
          phoneNumber: '03001232728',
          mail: 'killgoto@gmail.com',
          driverID: 'B5456',
          userID: '',
          tripStatus: false
        },
        {
          name: 'nuriel',
          lastName: 'batzur',
          age: '32',
          id: '785459586',
          licenceNum: 'SDG1S2G31SR',
          licenceType: '1',
          phoneNumber: '03001232922',
          mail: 'babayaga@gmail.com',
          driverID: 'B6666',
          userID: '',
          tripStatus: false
        },
        {
          name: 'yoram',
          lastName: 'gaon',
          age: '34',
          id: '125256588',
          licenceNum: 'SR21GS3R1G3',
          licenceType: '1',
          phoneNumber: '03001233031',
          mail: 'migaga@gmail.com',
          driverID: 'B8758',
          userID: '',
          tripStatus: false
        },
        {
          name: 'daniel',
          lastName: 'rosenberg',
          age: '47',
          id: '0112523365',
          licenceNum: 'SR51G63SR51',
          licenceType: '2',
          phoneNumber: '03001233233',
          mail: 'guliver@gmail.com',
          driverID: 'B6395',
          userID: '',
          tripStatus: false
        },
        {
          name: 'hen',
          lastName: 'tal',
          age: '65',
          id: '652879655',
          licenceNum: '54SR1G651TF',
          licenceType: '3',
          phoneNumber: '03001233435',
          mail: 'gangrty@gmail.com',
          driverID: 'B4142',
          userID: '',
          tripStatus: false
        },
        {
          name: 'glad',
          lastName: 'landaw',
          age: '56',
          id: '636596878',
          licenceNum: 'A04F4F54E21',
          licenceType: '1',
          phoneNumber: '03001233637',
          mail: 'glad@gmail.com',
          driverID: 'B4425',
          userID: '',
          tripStatus: false
        },
        {
          name: 'shem',
          lastName: 'bar',
          age: '62',
          id: '412457885',
          licenceNum: 'A1EF4A12E65',
          licenceType: '2',
          phoneNumber: '03001233839',
          mail: 'shem@gmail.com',
          driverID: 'B5563',
          userID: '',
          tripStatus: false
        },
        {
          name: 'shimon',
          lastName: 'kahalani',
          age: '48',
          id: '26859699',
          licenceNum: 'AE842FA54E1',
          licenceType: '1',
          phoneNumber: '03001235568',
          mail: 'shimon@gmail.com',
          driverID: 'B1158',
          userID: '',
          tripStatus: false
        }
      ],
      vehicle: [
        {
          truckNum: 'N34F45',
          truckID: 'A-01',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '113513',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34DE5',
          truckID: 'A-02',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '3513',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3R445',
          truckID: 'A-03',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '12154',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDFF45',
          truckID: 'A-04',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '12145',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'M34F45',
          truckID: 'A-05',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '4544',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'M34545',
          truckID: 'A-06',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '351254',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34645',
          truckID: 'A-07',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '45674',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34745',
          truckID: 'A-08',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '56525',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34845',
          truckID: 'B-01',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '53213',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34945',
          truckID: 'B-02',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '651632',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N30045',
          truckID: 'B-03',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '212313',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3KJ45',
          truckID: 'B-04',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '626262',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NGFF45',
          truckID: 'B-05',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '20005',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NJHF45',
          truckID: 'B-06',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '896588',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NSAF45',
          truckID: 'B-07',
          licenceType: '2',
          trailerAvailable: '0',
          addomer: '102325',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NVBF45',
          truckID: 'B-08',
          licenceType: '2',
          trailerAvailable: '0',
          addomer: '568412',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NVCF45',
          truckID: 'B-09',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '65165',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NHJF45',
          truckID: 'B-10',
          licenceType: '2',
          trailerAvailable: '0',
          addomer: '87465',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NKUF45',
          truckID: 'B-11',
          licenceType: '2',
          trailerAvailable: '0',
          addomer: '1651',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34FPO',
          truckID: 'B-12',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '98762',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3OP45',
          truckID: 'C-01',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '51321',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34PO5',
          truckID: 'C-02',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '351321',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34FPP',
          truckID: 'C-03',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '65262',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34OO5',
          truckID: 'C-04',
          licenceType: '3',
          trailerAvailable: '0',
          addomer: '786854',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDGF45',
          truckID: 'C-05',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '22333',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDDF45',
          truckID: 'C-06',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '15155',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3FF45',
          truckID: 'C-07',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '61354',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        }
      ],
      trailers: [
        {
          trailerNumber: 'SBRG3GG',
          trailerID: 'T1',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SSSF3B4',
          trailerID: 'T2',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRSSB4',
          trailerID: 'T3',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBSFSB4',
          trailerID: 'T4',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SSRF3B4',
          trailerID: 'T5',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'HBRF3B4',
          trailerID: 'T6',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SHRF3B4',
          trailerID: 'T7',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBHF3B4',
          trailerID: 'T8',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRH3B4',
          trailerID: 'T9',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRFHB4',
          trailerID: 'T10',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRF3H4',
          trailerID: 'T11',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        }
      ]
    },
    hunterbg: {
      drivers: [
        {
          name: 'jhoni',
          lastName: 'dui',
          age: '56',
          id: '3125007898',
          licenceNum: '4R55Q2S5R4T',
          licenceType: '2',
          phoneNumber: '03008881213',
          mail: 'dorma@gmail.com',
          driverID: 'C1321',
          userID: '',
          tripStatus: false
        },
        {
          name: 'dav',
          lastName: 'migdal',
          age: '45',
          id: '312800456',
          licenceNum: '4Y4Q24S4FG4',
          licenceType: '2',
          phoneNumber: '03008881415',
          mail: 'vlimdfg@gmail.com',
          driverID: 'C1232',
          userID: '',
          tripStatus: false
        },
        {
          name: 'kakilovitch',
          lastName: 'malginov',
          age: '58',
          id: '3100687888',
          licenceNum: '2W3Q235E4RD',
          licenceType: '2',
          phoneNumber: '03888231617',
          mail: 'vmavae@gmail.com',
          driverID: 'C2121',
          userID: '',
          tripStatus: false
        },
        {
          name: 'rafael',
          lastName: 'bazumet',
          age: '63',
          id: '3125007895',
          licenceNum: '32GQ23S524G',
          licenceType: '1',
          phoneNumber: '03001231888',
          mail: 'mamya@gmail.com',
          driverID: 'C4545',
          userID: '',
          tripStatus: false
        },
        {
          name: 'shabi',
          lastName: 'daniel',
          age: '77',
          id: '012009648',
          licenceNum: 'S3R2G1SQ2R5',
          licenceType: '1',
          phoneNumber: '03008882122',
          mail: 'namayma@gmail.com',
          driverID: 'C4857',
          userID: '',
          tripStatus: false
        },
        {
          name: 'rovad',
          lastName: 'shalev',
          age: '23',
          id: '014001859',
          licenceNum: 'SR6541G26Q2',
          licenceType: '2',
          phoneNumber: '03008882324',
          mail: 'nodemanmd@gmail.com',
          driverID: 'C9878',
          userID: '',
          tripStatus: false
        },
        {
          name: 'yonnaan',
          lastName: 'cohen',
          age: '34',
          id: '165800652',
          licenceNum: '15SQ2G15GG',
          licenceType: '2',
          phoneNumber: '03008882526',
          mail: 'noobfotalml@gmail.com',
          driverID: 'C5465',
          userID: '',
          tripStatus: false
        },
        {
          name: 'avrahamson',
          lastName: 'mrfison',
          age: '42',
          id: '006969631',
          licenceNum: '35ES41Q2625',
          licenceType: '3',
          phoneNumber: '88801232728',
          mail: 'kilmlgoto@gmail.com',
          driverID: 'C5456',
          userID: '',
          tripStatus: false
        },
        {
          name: 'nurieli',
          lastName: 'batzur',
          age: '32',
          id: '700459586',
          licenceNum: 'SDG1S2G31Q2',
          licenceType: '1',
          phoneNumber: '03008882922',
          mail: 'babmymayaga@gmail.com',
          driverID: 'C6666',
          userID: '',
          tripStatus: false
        },
        {
          name: 'yoram',
          lastName: 'gaon',
          age: '34',
          id: '125250088',
          licenceNum: 'SR21GQ2R1G3',
          licenceType: '1',
          phoneNumber: '03001288831',
          mail: 'migagamy@gmail.com',
          driverID: 'C8758',
          userID: '',
          tripStatus: false
        },
        {
          name: 'gabi',
          lastName: 'rosenberg',
          age: '47',
          id: '0002523365',
          licenceNum: 'SQ21G63SR51',
          licenceType: '2',
          phoneNumber: '03888233233',
          mail: 'gufrfrliver@gmail.com',
          driverID: 'C6395',
          userID: '',
          tripStatus: false
        },
        {
          name: 'haniel',
          lastName: 'tal',
          age: '65',
          id: '652811655',
          licenceNum: '54SR1GQ21TF',
          licenceType: '3',
          phoneNumber: '03001238885',
          mail: 'gangrffty@gmail.com',
          driverID: 'C4142',
          userID: '',
          tripStatus: false
        },
        {
          name: 'hananel',
          lastName: 'landaw',
          age: '56',
          id: '636591178',
          licenceNum: 'A0Q24F54E21',
          licenceType: '1',
          phoneNumber: '03088833637',
          mail: 'gfflad@gmail.com',
          driverID: 'C4425',
          userID: '',
          tripStatus: false
        },
        {
          name: 'jhon',
          lastName: 'bar',
          age: '62',
          id: '412111885',
          licenceNum: 'A1EF4A12Q25',
          licenceType: '2',
          phoneNumber: '03001238889',
          mail: 'shemb@gmail.com',
          driverID: 'C5563',
          userID: '',
          tripStatus: false
        },
        {
          name: 'jeck',
          lastName: 'kahalani',
          age: '48',
          id: '26851699',
          licenceNum: 'AE842Q254E1',
          licenceType: '1',
          phoneNumber: '03008885568',
          mail: 'shimonDE@gmail.com',
          driverID: 'C1158',
          userID: '',
          tripStatus: false
        }
      ],
      vehicle: [
        {
          truckNum: 'NZ4F45',
          truckID: 'A01',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '1130113',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3ODE5',
          truckID: 'A02',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '358813',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3R845',
          truckID: 'A03',
          licenceType: '1',
          trailerAvailable: '1',
          addomer: '121854',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDFF05',
          truckID: 'A04',
          licenceType: '1',
          trailerAvailable: '1',
          addomer: '126145',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'M34FP5',
          truckID: 'A05',
          licenceType: '1',
          trailerAvailable: '1',
          addomer: '4577644',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'M345R5',
          truckID: 'A06',
          licenceType: '1',
          addomer: '35154',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N14645',
          truckID: 'A07',
          licenceType: '1',
          trailerAvailable: '1',
          addomer: '4577674',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3A745',
          truckID: 'A08',
          licenceType: '1',
          trailerAvailable: '2',
          addomer: '555525',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3G845',
          truckID: 'B01',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '5213',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N74945',
          truckID: 'B02',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '61632',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N39045',
          truckID: 'B03',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '22313',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3K045',
          truckID: 'B04',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '62262',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NQFF45',
          truckID: 'B05',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '200085',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NWHF45',
          truckID: 'B06',
          licenceType: '2',
          trailerAvailable: '1',
          addomer: '89588',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NSAF85',
          truckID: 'B07',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '10225',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N7VBF5',
          truckID: 'B08',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '56412',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NVC745',
          truckID: 'B09',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '665165',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NHJR45',
          truckID: 'B10',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '8765',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NKUI45',
          truckID: 'B11',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '167751',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3VFPO',
          truckID: 'B12',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '97762',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3OPB5',
          truckID: 'C1',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '513299',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34PJ5',
          truckID: 'C2',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '35131',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34FDP',
          truckID: 'C3',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '65762',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NF4OO5',
          truckID: 'C4',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '78854',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'GDGF45',
          truckID: 'C5',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '223363',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NGDF45',
          truckID: 'C6',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '1555',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3FG45',
          truckID: 'C7',
          licenceType: '3',
          trailerAvailable: '1',
          addomer: '6354',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        }
      ],
      trailers: [
        {
          trailerNumber: 'SBRG3GG',
          trailerID: 'T1',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SSSF3B4',
          trailerID: 'T2',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRSSB4',
          trailerID: 'T3',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBSFSB4',
          trailerID: 'T4',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SSRF3B4',
          trailerID: 'T5',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'HBRF3B4',
          trailerID: 'T6',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SHRF3B4',
          trailerID: 'T7',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBHF3B4',
          trailerID: 'T8',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRH3B4',
          trailerID: 'T9',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRFHB4',
          trailerID: 'T10',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRF3H4',
          trailerID: 'T11',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        }
      ]
    },
    cita: {
      drivers: [
        {
          name: 'ziv',
          lastName: 'magnezi',
          age: '27',
          id: '3125098898',
          licenceNum: '4R55T5S5R4T',
          licenceType: '2',
          phoneNumber: '03008721213',
          mail: 'ziv@gmail.com',
          driverID: 'CA1321',
          userID: 'z9T4blDxNpSzpDQpHDMYkHkx2ld2',
          tripStatus: false
        },
        {
          name: 'yudit',
          lastName: 'magnezi',
          age: '49',
          id: '312980456',
          licenceNum: '4Y4T54S4FG4',
          licenceType: '2',
          phoneNumber: '03088172415',
          mail: 'yudit@gmail.com',
          driverID: 'CA1232',
          userID: 'n9y8lXzORyMQaUdhlPSGtFCgHF63',
          tripStatus: false
        },
        {
          name: 'katch',
          lastName: 'maldginov',
          age: '58',
          id: '3980687888',
          licenceNum: 'T53Q235E4RD',
          licenceType: '2',
          phoneNumber: '03728231617',
          mail: 'vmaXvae@gmail.com',
          driverID: 'CA2121',
          userID: '',
          tripStatus: false
        },
        {
          name: 'ref',
          lastName: 'bazudmet',
          age: '63',
          id: '3125009895',
          licenceNum: '32GT53S524G',
          licenceType: '1',
          phoneNumber: '03001721888',
          mail: 'mamyXa@gmail.com',
          driverID: 'CA4545',
          userID: '',
          tripStatus: false
        },
        {
          name: 'rif',
          lastName: 'danidel',
          age: '77',
          id: '012098648',
          licenceNum: 'S3R2GT5Q2R5',
          licenceType: '1',
          phoneNumber: '03008887222',
          mail: 'namaymXa@gmail.com',
          driverID: 'CA4857',
          userID: '',
          tripStatus: false
        },
        {
          name: 'donald',
          lastName: 'sdhalev',
          age: '23',
          id: '014098859',
          licenceNum: 'SRT541G26Q2',
          licenceType: '2',
          phoneNumber: '03007282324',
          mail: 'nodeXmanmd@gmail.com',
          driverID: 'CA9878',
          userID: '',
          tripStatus: false
        },
        {
          name: 'yodn',
          lastName: 'codhen',
          age: '34',
          id: '165898652',
          licenceNum: '15SQ2T55GG',
          licenceType: '2',
          phoneNumber: '03008882726',
          mail: 'noobfotaXlml@gmail.com',
          driverID: 'CA5465',
          userID: '',
          tripStatus: false
        },
        {
          name: 'mtson',
          lastName: 'mrdfison',
          age: '42',
          id: '009869631',
          licenceNum: '35T541Q2625',
          licenceType: '3',
          phoneNumber: '88807232728',
          mail: 'kilmXlgoto@gmail.com',
          driverID: 'CA5456',
          userID: '',
          tripStatus: false
        },
        {
          name: 'mat',
          lastName: 'badtzur',
          age: '32',
          id: '700498586',
          licenceNum: 'SDG1ST531Q2',
          licenceType: '1',
          phoneNumber: '03008887222',
          mail: 'babmyXmayaga@gmail.com',
          driverID: 'CA6666',
          userID: '',
          tripStatus: false
        },
        {
          name: 'ram',
          lastName: 'gadon',
          age: '34',
          id: '125250988',
          licenceNum: 'SR21GQ2T5G3',
          licenceType: '1',
          phoneNumber: '03001287231',
          mail: 'migagamXy@gmail.com',
          driverID: 'CA8758',
          userID: '',
          tripStatus: false
        },
        {
          name: 'ford',
          lastName: 'rodsenberg',
          age: '47',
          id: '0902523365',
          licenceNum: 'T521G63SR51',
          licenceType: '2',
          phoneNumber: '72888233233',
          mail: 'Xgufrfrliver@gmail.com',
          driverID: 'CA6395',
          userID: '',
          tripStatus: false
        },
        {
          name: 'hanan',
          lastName: 'tdal',
          age: '65',
          id: '652819855',
          licenceNum: '54SR1GT51TF',
          licenceType: '3',
          phoneNumber: '03002712885',
          mail: 'ganXgrffty@gmail.com',
          driverID: 'CA4142',
          userID: '',
          tripStatus: false
        },
        {
          name: 'hsdel',
          lastName: 'laandaw',
          age: '56',
          id: '639891178',
          licenceNum: 'A0QT5F54E21',
          licenceType: '1',
          phoneNumber: '07288833637',
          mail: 'gXfflad@gmail.com',
          driverID: 'CA4425',
          userID: '',
          tripStatus: false
        },
        {
          name: 'shrek',
          lastName: 'baar',
          age: '62',
          id: '412119885',
          licenceNum: 'A1EF4AT5Q25',
          licenceType: '2',
          phoneNumber: '03007238889',
          mail: 'sheXmb@gmail.com',
          driverID: 'CA5563',
          userID: '',
          tripStatus: false
        },
        {
          name: 'jeri',
          lastName: 'kahaalani',
          age: '48',
          id: '29851699',
          licenceNum: 'AE84T5254E1',
          licenceType: '1',
          phoneNumber: '03728885568',
          mail: 'sXhimonDE@gmail.com',
          driverID: 'CA1158',
          userID: '',
          tripStatus: false
        }
      ],
      vehicle: [
        {
          truckNum: 'N34FAX',
          truckID: '1',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '1135213',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'ZZN345',
          truckID: '2',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '343513',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3V445',
          truckID: '3',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '129154',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDF145',
          truckID: '4',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '125145',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'M34XX5',
          truckID: '5',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '3454544',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'MZZ545',
          truckID: '6',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '31254',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NQQ645',
          truckID: '7',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '456674',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34RR5',
          truckID: '8',
          licenceType: '1',
          trailerAvailable: '0',
          addomer: '569925',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N3WA45',
          truckID: '9',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '532613',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34II5',
          truckID: '10',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '651632',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'I30I45',
          truckID: '11',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '21313',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N37J75',
          truckID: '12',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '626262',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N7FF75',
          truckID: '13',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '200605',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: '8JHF48',
          truckID: '14',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '89588',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: '7S7F45',
          truckID: '15',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '10225',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N1V1F4',
          truckID: '16',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '56412',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NV0F04',
          truckID: '17',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '650165',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'HHJF45',
          truckID: '18',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '87465',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NKIFI5',
          truckID: '19',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '1651',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'U34UPO',
          truckID: '20',
          licenceType: '2',
          trailerAvailable: '2',
          addomer: '98762',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N33P35',
          truckID: '21',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '51321',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N34P00',
          truckID: '22',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '351321',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: '034F0P',
          truckID: '23',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '65262',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N33O35',
          truckID: '24',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '786854',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDG555',
          truckID: '25',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '22333',
          onTrip: true,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'NDFF11',
          truckID: '26',
          licenceType: '3',
          trailerAvailable: '2',
          addomer: '15155',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          truckNum: 'N2F245',
          truckID: '27',
          licenceType: '3',
          trailerAvailable: '0',
          addomer: '61354',
          onTrip: false,
          status: {
            airCompresor: {
              status: true,
              dateModified: ''
            },
            airLines: {
              status: true,
              dateModified: ''
            },
            battery: {
              status: true,
              dateModified: ''
            },
            beltsAndHoses: {
              status: true,
              dateModified: ''
            },
            body: {
              status: true,
              dateModified: ''
            },
            brakeAccessories: {
              status: true,
              dateModified: ''
            },
            brakesParking: {
              status: true,
              dateModified: ''
            },
            brakesService: {
              status: true,
              dateModified: ''
            },
            clutch: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            driverLine: {
              status: true,
              dateModified: ''
            },
            engine: {
              status: true,
              dateModified: ''
            },
            exhaust: {
              status: true,
              dateModified: ''
            },
            fifthWheel: {
              status: true,
              dateModified: ''
            },
            fluidLevel: {
              status: true,
              dateModified: ''
            },
            frameAndAssembly: {
              status: true,
              dateModified: ''
            },
            frontAxle: {
              status: true,
              dateModified: ''
            },
            fuleTanks: {
              status: true,
              dateModified: ''
            },
            generator: {
              status: true,
              dateModified: ''
            },
            horn: {
              status: true,
              dateModified: ''
            },
            lightHead: {
              status: true,
              dateModified: ''
            },
            lightStop: {
              status: true,
              dateModified: ''
            },
            lightTail: {
              status: true,
              dateModified: ''
            },
            lightDash: {
              status: true,
              dateModified: ''
            },
            lightTurnIndicators: {
              status: true,
              dateModified: ''
            },
            mirrors: {
              status: true,
              dateModified: ''
            },
            muffler: {
              status: true,
              dateModified: ''
            },
            oilLevel: {
              status: true,
              dateModified: ''
            },
            radiatorLevel: {
              status: true,
              dateModified: ''
            },
            rearEnd: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            fireExtinguisher: {
              status: true,
              dateModified: ''
            },
            flags: {
              status: true,
              dateModified: ''
            },
            flares: {
              status: true,
              dateModified: ''
            },
            fusees: {
              status: true,
              dateModified: ''
            },
            reflectiveTriangles: {
              status: true,
              dateModified: ''
            },
            spareBulbs: {
              status: true,
              dateModified: ''
            },
            spareFuses: {
              status: true,
              dateModified: ''
            },
            spareSealBeam: {
              status: true,
              dateModified: ''
            },
            starter: {
              status: true,
              dateModified: ''
            },
            steering: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            tireChains: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            transmission: {
              status: true,
              dateModified: ''
            },
            tripRecorder: {
              status: true,
              dateModified: ''
            },
            weelsAndRim: {
              status: true,
              dateModified: ''
            },
            windows: {
              status: true,
              dateModified: ''
            },
            windshieldsWipers: {
              status: true,
              dateModified: ''
            }
          }
        }
      ],
      trailers: [
        {
          trailerNumber: 'SBRG3GG',
          trailerID: 'T1',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SSSF3B4',
          trailerID: 'T2',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRSSB4',
          trailerID: 'T3',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBSFSB4',
          trailerID: 'T4',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SSRF3B4',
          trailerID: 'T5',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'HBRF3B4',
          trailerID: 'T6',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SHRF3B4',
          trailerID: 'T7',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBHF3B4',
          trailerID: 'T8',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRH3B4',
          trailerID: 'T9',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRFHB4',
          trailerID: 'T10',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        },
        {
          trailerNumber: 'SBRF3H4',
          trailerID: 'T11',
          onTrip: false,
          status: {
            brakeConnections: {
              status: true,
              dateModified: ''
            },
            brakes: {
              status: true,
              dateModified: ''
            },
            couplingDevices: {
              status: true,
              dateModified: ''
            },
            couplingKingPin: {
              status: true,
              dateModified: ''
            },
            doors: {
              status: true,
              dateModified: ''
            },
            hitch: {
              status: true,
              dateModified: ''
            },
            landingGear: {
              status: true,
              dateModified: ''
            },
            lights: {
              status: true,
              dateModified: ''
            },
            reflectors: {
              status: true,
              dateModified: ''
            },
            roof: {
              status: true,
              dateModified: ''
            },
            suspensionSystem: {
              status: true,
              dateModified: ''
            },
            straps: {
              status: true,
              dateModified: ''
            },
            tarpulin: {
              status: true,
              dateModified: ''
            },
            tires: {
              status: true,
              dateModified: ''
            },
            wheelsAndRim: {
              status: true,
              dateModified: ''
            }
          }
        }
      ]
    }
  };

  // AsyncStorage.getItem('userData')
  //   .then((userData) => {
  //     const test = JSON.parse(userData);
  //     const blaa = test.token;
  //     post(blaa);
  //   });

  // const post = (token) => {
  //   axios.post(`https://dvir-project-server.firebaseio.com/companysData.json?auth=${token}`, companys)
  //     .then((res) => console.log(res))
  //     .catch((err) => console.log(err));
  // };


// const get = async () => {
//   await axios.get('https://react-burger-app-9b9c5.firebaseio.com/.json')
//     .then((res) => console.log(res))
//     .catch((err) => console.log(err));
// };

// const del = () => {
//   axios.delete('https://react-burger-app-9b9c5.firebaseio.com/-LyVgH73c_k6vdM0Qgvh.json')
//     .then((res) => console.log(res))
//     .catch((err) => console.log(err));
// };

// const put = async () => {
//   const newDATA = {
//     name: 'avi',
//     lastName: 'shami',
//     age: '32',
//     car: 'volvo'
//   };
//   await axios.put('https://react-burger-app-9b9c5.firebaseio.com/users.json', newDATA)
//     .then((res) => console.log(res))
//     .catch((err) => console.log(err));
// };


  return (
    <View>
      <Text>smvsmv</Text>
      {/* <Button title="post" onPress={() => post()} />
      <Button title="get" onPress={() => get()} />
      <Button title="delete" onPress={() => del()} />
      <Button title="put" onPress={() => put()} /> */}
    </View>
  );
};
export default TestScreen;


// const blaa = () => {
//   AsyncStorage.getItem('firstTimeUser')
//     .then((firstTimeUser) => {
//       console.log(firstTimeUser);
//     });
// };

// const blaa = async () => {
//   const firstTimeUser = await AsyncStorage.getItem('firstTimeUser');
//   console.log(firstTimeUser);
// };